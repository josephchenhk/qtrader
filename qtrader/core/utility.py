...
# ADD(joseph): Specify paths in use
setting_path = os.path.dirname(os.path.realpath(__file__))
main_path = str(Path(setting_path).parent)
strategy_path = os.path.join(main_path, "strategy")
...
def _get_trader_dir(temp_name: str) -> Tuple[Path, Path]:
    ...
    """
    # CHG(joseph): the first priority is to use main_path (
    # which was configured in qtrader.setting.__init__.py) as the working
    # directory. If no setting folder in main_path, we try to use current
    # working directory instead.
    main_path = Path(PATH["main_path"])
    temp_path = main_path.joinpath(temp_name)
    if main_path.exists():
        return main_path, temp_path

    # If setting folder exists in current working directory,
...
TRADER_DIR, TEMP_DIR = _get_trader_dir("setting")
