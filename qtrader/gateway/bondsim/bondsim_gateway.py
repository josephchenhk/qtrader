# -*- coding: utf-8 -*-
# @Time    : 10/9/2020 11:40 AM
# @Author  : Joseph Chen
# @Email   : josephchenhk@gmail.com
# @FileName: bondsim_gateway.py
# @Software: PyCharm
...
from qtrader.core.constant import Direction, Exchange, Product, Status
from qtrader.core.event import EVENT_TIMER
from qtrader.gateway import BaseGateway
from qtrader.core.object import (
    TickData,
    OrderData,
    TradeData,
    AccountData,
...
from qtrader.gateway.bondsim.bondsim_client import SimulatorClient

class BondsimGateway(BaseGateway):
    """"""
    ...
    def __init__(self, event_engine):
        """Constructor"""
        super(BondsimGateway, self).__init__(event_engine, "BONDSIM")
        ...
        self.thread.setName("BondsimGatewayQueryDataThread")
    ...
    def load_data(self, symbols:list, data_handlers:dict, factsheets:dict):
        """ ADD(joseph): 加载历史数据、静态数据作为数据源"""
        self.symbols = symbols  # symbols monitored by the gateway
        self.data_handlers = data_handlers
        self.factsheets = factsheets

    def update_timer(self, time: datetime):
        """ ADD(joseph): 更新gateway的时钟，据此分发market data"""
        self.cur_time = time
    ...
    def generate_snapshot(self):
        ...
        for symbol in self.symbols:
            handler = self.data_handlers["{}_handler".format(symbol)]
            tick_data = handler.read_tick()
            if tick_data is None:
                snapshot[symbol] = None
                continue
            last_tick_data = tick_data
            while tick_data["datetime"] < self.cur_time:
                last_tick_data = tick_data
                tick_data = handler.read_tick()
            snapshot[symbol] = last_tick_data
    ...
    def connect_quote(self):
        ...
        """
        self.quote_ctx = SimulatorClient()

        class QuoteHandler:
            name = "QUOTE"
            gateway = self

            def on_recv_rsp(self, rsp_dict):
                self.gateway.process_quote(rsp_dict)
        ...
        class OrderBookHandler:
            ...
            def on_recv_rsp(self, rsp_dict):
                self.gateway.process_orderbook(rsp_dict)
    ...
    def get_tick(self, code):
        """
        Get tick buffer (If no buffer, create a new instance).
        """
        tick = self.ticks.get(code, None)
        # symbol, exchange = convert_symbol_futu2vt(code)
        symbol, exchange = code, type('Exchange', (object,), dict(value="BONDSIM"))
        if not tick:
            tick = TickData(
                symbol=symbol,
                exchange=exchange,
    ...
    def process_quote(self, data):
        ...
        for symbol in data:
            ...
            tick.datetime = parse(data[symbol]['datetime'])
            tick.last_price = data[symbol]['trd']
            tick.volume = data[symbol]['trd_size']
    ...
    def process_orderbook(self, data):
        ...
        for symbol in data:
            tick = self.get_tick(symbol)
            tick.datetime = parse(data[symbol]['datetime'])
            tick.bid_price_1 = data[symbol]['bid']
            tick.bid_volume_1 = data[symbol]['bid_size']
            tick.ask_price_1 = data[symbol]['ask']
            tick.ask_volume_1 = data[symbol]['ask_size']
