...
result_path = "results/2021-09-24 10-49-10.569006/result.csv"
...
with pd.ExcelWriter(result.parent.joinpath("stats.xlsx")) as writer:
    ...
    pd.concat([win_trades_df, loss_trades_df, flat_trades_df]).groupby("open_session").pnl.agg(["sum", "count", "mean"]).to_excel(
        writer,
    ...
    holding_period_df = pd.concat([win_trades_holding_period.T, loss_trades_holding_period.T, flat_trades_holding_period.T])
    holding_period_df.index = ["win_trades", "loss_trades", "flat_trades"]
